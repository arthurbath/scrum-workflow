...
flowchart TD
    ...
    B -.- NOTE_B["💬 Between Backlog and the ready-for-design / ready-for-development statuses, it would be beneficial for POs to have a status such as Ready for Refinement or Ready for Pointing. That would help them organize which tickets still need PO work before they are ready to take into scrum ceremonies."]
    T -.- NOTE_T["💬 To make it clear who this ticket is ready to be picked up by, this should use a more explicit status name such as Ready for Design or Ready for Development. Ideally, To Do should be split into two separate statuses: one for design and one for development."]
    ...
    HOLD -.- NOTE_HOLD["💬 I prefer to use Jira flags to indicate blockages, since a blockage can happen to a ticket in almost any status. That also gives us a strong visual highlight in Jira to show which tickets are blocked at each stage of production."]
    ...
    subgraph DE["Development Environment"]
        direction TB
        CADEV -.- NOTE_CADEV["💬 I think this should be split into Ready for Code Approval and In Code Approval, since plenty of work can stack up that is ready to enter code approval without actually being picked up."]
    ...
    subgraph SE["Staging Environment"]
        ...
        CASTG -.- NOTE_CASTG["💬 Ditto here: split this into Ready for Code Approval and In Code Approval so queued work is visible before someone actually picks it up."]
    ...
    subgraph PE["Production Environment"]
        ...
        IPROD -.- NOTE_IPROD["💬 Why does this status need to exist? After a ticket is code-approved on production, should it just move directly to Done instead?"]
    end
